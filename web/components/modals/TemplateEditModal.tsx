...
export function TemplateEditModal({ template, onClose, onSaved }: TemplateEditModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm overflow-y-auto">
      <div className="bg-card border border-cardBorder rounded-2xl w-full max-w-3xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        {/* Header */}
        <div className="p-6 border-b border-cardBorder flex items-center justify-between bg-slate-900/50">
          <div>
            <h2 className="text-xl font-bold text-text flex items-center gap-2">
              <Layers className="w-5 h-5 text-primary" />
              {isEditing ? `Edit Template: ${template.name}` : 'Create Custom EPR Template'}
            </h2>
            <p className="text-xs text-textMuted mt-0.5">
              Define top-level attributes and repeated table columns for structured voice extraction.
            </p>
          </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 rounded-xl bg-slate-900/60 border border-cardBorder">
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2 rounded-xl bg-background border border-cardBorder text-sm text-text font-semibold focus:outline-none focus:border-primary"
              />
            </div>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2.5">
              {fields.map((f, idx) => (
                <div key={f.id || idx} className="p-3.5 rounded-xl bg-slate-900/50 border border-cardBorder grid grid-cols-1 md:grid-cols-12 gap-2.5 items-center">
                  <div className="md:col-span-5">
                    <label className="text-[10px] text-textSubtle font-medium block mb-0.5">Field Label</label>
                    <input
                      type="text"
                      value={f.name}
                      onChange={(e) => updateField(idx, 'name', e.target.value)}
                      className="w-full px-3 py-1.5 rounded-lg bg-background border border-cardBorder text-xs text-text focus:outline-none focus:border-primary font-medium"
                    />
                  </div>

                  <div className="md:col-span-4">
                    <label className="text-[10px] text-textSubtle font-medium block mb-0.5">AI Key</label>
                    <input
                      type="text"
                      value={f.extractionKey}
                      onChange={(e) => updateField(idx, 'extractionKey', e.target.value)}
                      className="w-full px-3 py-1.5 rounded-lg bg-background border border-cardBorder text-xs text-textSubtle focus:outline-none focus:border-primary font-mono text-[11px]"
                    />
                  </div>

                  <div className="md:col-span-2">
                    <label className="text-[10px] text-textSubtle font-medium block mb-0.5">Type</label>
                    <select
                      value={f.type}
          ...
          <div className="space-y-4 pt-4 border-t border-cardBorder">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-textMuted">Enable for hourly logs, inspection intervals, or multi-row entries.</p>
                </div>
              </div>
              <input
                type="checkbox"
            ...
            {hasTable && (
              <div className="p-4 rounded-xl bg-slate-900/60 border border-cardBorder space-y-4">
                <div>
                  <label className="block text-[11px] font-semibold uppercase text-textSubtle mb-1">
                    Table Header Title
                  </label>
                ...
                <div className="space-y-2.5">
                  <div className="flex items-center justify-between">
                    <label className="text-[11px] font-semibold uppercase text-textSubtle">Table Columns</label>
                    <button
                  ...
                  {tableFields.map((col, idx) => (
                    <div key={col.id || idx} className="p-3 rounded-xl bg-slate-900/50 border border-cardBorder grid grid-cols-1 md:grid-cols-12 gap-2.5 items-center">
                      <div className="md:col-span-5">
                        <input
                          type="text"
                          value={col.name}
